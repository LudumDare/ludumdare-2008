...
function twitch_streams_get( $game_name, $api_key ) {
	...
	do {
		...
		do {
			...
			$api_url = "https://api.twitch.tv/kraken/streams/?game=" . $game_name . "&limit=" . $limit . "&offset=" . $offset . "&api_version=3";
...
function beam_streams_get( $game_name ) {
	...
	$api_url .= "?order=online%3Adesc%2CviewersCurrent%3Adesc%2CviewersTotal%3Adesc&fields=id%2Ctoken%2Conline%2CviewersCurrent%2Cname%2CnumFollowers%2Cthumbnail%2Ctype&where=suspended.eq.0&scope=names&q=";
	$api_response = @file_get_contents($api_url); // @ surpresses PHP error: http://stackoverflow.com/a/15685966
