...
// TODO: Store Peak Users (use steam UID to determine if stream is the same) //
...
// MAIN //
{
	...
	$beam_streams = beam_streams_get( /*"development"*/ /*"ludum-dare"*/ 83453 ); // not sure if $game_name is a + or - //
...
//					ID SERIAL PRIMARY KEY,
		...
		if ( $hitbox_streams !== NULL ) {
			foreach ( $hitbox_streams['livestream'] as $value ) {
				...
				$channel_avatar = mysqli_real_escape_string($db,'http://edge.sf.hitbox.tv' . trim($value['channel']['user_logo']));
		...
		if ( $beam_streams !== NULL ) {
			foreach ( $beam_streams as $value ) {
				if ( $value['online'] === true ) {
					...
					$channel_followers = intval($value['numFollowers']);
					$media_viewers = intval($value['viewersCurrent']);
					...
					$channel_embed_url = mysqli_real_escape_string($db,"https://beam.pro/embed/player/".trim($value['token']));
