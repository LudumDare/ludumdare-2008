<?php
if( !class_exists('ManageDarenatePlus') ):
	class ManageDarenatePlus{
		function ManageDarenatePlus() { //constructor
		...
		function Actions(){
			...
			if( $_GET['action'] == 'delete' || $_GET['delete']):
				...
				$mngpg = get_option('siteurl').'/wp-admin/admin.php?page=DarenatePlus';
		...
		function Manage(){
			...
			?>
            <div class="wrap">
            	<h2><?php _e('Manage Donations', 'dplus');?></h2>
                <form id="donate-filter" action="<?php echo $mngpg;?>" method="get"><input type="hidden" name="page" value="DarenatePlus" />
<p class="search-box">
	<label class="screen-reader-text" for="page-search-input"><?php _e('Search Donations:','dplus');?></label>
	<input type="text" id="donate-search-input" name="s" value="" />
	<input type="submit" value="<?php _e('Search Donations','dplus');?>" class="button" />
...
if( class_exists('ManageDarenatePlus') )
	$manageDP = new ManageDarenatePlus();